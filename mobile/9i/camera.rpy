init -1600 python:
    ...
    def camera_move(x, y, z, rotate=0, duration=0, warper=None, subpixel=True, loop=False, x_express=None, y_express=None, z_express=None, rotate_express=None, **kwargs):
        """
         :doc: camera
        ...
        camera_moves([(x, y, z, rotate, duration, warper or _viewers.default_warper, ),], subpixel=subpixel, loop=loop, x_express=x_express, y_express=y_express, z_express=z_express, rotate_express=rotate_express, **kwargs)

    def camera_relative_move(x, y, z, rotate=0, duration=0, warper=None, subpixel=True, loop=False, x_express=None, y_express=None, z_express=None, rotate_express=None, **kwargs):
        """
         :doc: camera
    ...
    def layer_move(layer, z, duration=0, warper=None, subpixel=True, loop=False, x_express=None, y_express=None, z_express=None, rotate_express=None, layer_express=None):
        """
         :doc: camera
        ...
        layer_moves(layer, [(z, duration, warper or _viewers.default_warper, ),], subpixel=subpixel, loop=loop, x_express=x_express, y_express=y_express, z_express=z_express, rotate_express=rotate_express, layer_express=layer_express)
    ...
    def all_moves(camera_check_points=None, layer_check_points=None, subpixel=True, play=True, x_loop=False, y_loop=False, z_loop=False, rotate_loop=False, x_express=None, y_express=None, z_express=None, rotate_express=None, camera_spline=False, **kwargs):
        ...
        start_xanchor = _FOCAL_LENGTH*_camera_x/(1920*_LAYER_Z) + .5
        start_yanchor = _FOCAL_LENGTH*_camera_y/(1080*_LAYER_Z) + .5
        ...
        for c in camera_check_points['x']:
            camera_check_points2['xanchor'].append((_FOCAL_LENGTH*c[0]/(1920 *_LAYER_Z) + .5, c[1], c[2], ))
        for c in camera_check_points['y']:
            camera_check_points2['yanchor'].append((_FOCAL_LENGTH*c[0]/(1080 *_LAYER_Z) + .5, c[1], c[2], ))
        ...
        for layer in _3d_layers:
            ...
            if play:
                ...
                if x_express:
                    xanchor += _FOCAL_LENGTH*x_express(st, st)/(1920 *_LAYER_Z)
                if y_express:
                    yanchor += _FOCAL_LENGTH*y_express(st, st)/(1080 *_LAYER_Z)
                if z_express:
                    z += z_express(st, st)
                if rotate_express:
        ...
        if play:
            ...
            _camera_x         = int(((xanchor-.5)*1920*_LAYER_Z)/_FOCAL_LENGTH)
            _camera_y         = int(((yanchor-.5)*1080*_LAYER_Z)/_FOCAL_LENGTH)
            _camera_z         = int(z)
    ...
    def _camera_trans(tran, st, at, camera_check_points, layer_check_points, layer_loop, x_loop, y_loop, z_loop, rotate_loop, subpixel, x_express, y_express, z_express, rotate_express, layer_express):
        ...
        if x_express:
            tran.xanchor += _FOCAL_LENGTH*x_express(st, at)/(1920*_LAYER_Z)
        if y_express:
            tran.yanchor += _FOCAL_LENGTH*y_express(st, at)/(1080*_LAYER_Z)
        if z_express:
            z += z_express(st, at)
        if rotate_express:
    ...
    frame:
        ...
        vbox:
            ...
            hbox:
                ...
                textbutton "time: [_viewers.time:>.2f] s" action Function(_viewers.edit_time)
                textbutton "<" action Function(_viewers.prev_time)
                textbutton ">" action Function(_viewers.next_time)
                bar adjustment ui.adjustment(range=7.0, value=_viewers.time, changed=_viewers.change_time) xalign 1. yalign .5
            hbox:
                style_group "action_editor_a"
                hbox:
                    textbutton "default warper" action _viewers.select_default_warper
                    textbutton "rot" action [SelectedIf(renpy.get_screen("_rot")), If(renpy.get_screen("_rot"), true=Hide("_rot"), false=Show("_rot"))]
                    textbutton "hide" action HideInterface()

                    if _viewers.sorted_keyframes:
                        textbutton "play" action [SensitiveIf(_viewers.sorted_keyframes), Function(_viewers.camera_viewer.play, play=True), Function(_viewers.transform_viewer.play, play=True), Hide("_action_editor"), Show("_action_editor", tab=tab, layer=layer, name=name, time=_viewers.sorted_keyframes[-1]), renpy.restart_interaction]
                    else:
                        textbutton "play" action [SensitiveIf(_viewers.sorted_keyframes), Function(_viewers.camera_viewer.play, play=True), Function(_viewers.transform_viewer.play, play=True), Hide("_action_editor"), Show("_action_editor", tab=tab, layer=layer, name=name), renpy.restart_interaction]
                    textbutton "clipboard" action Function(_viewers.put_clipboard)
                hbox:
                    xalign 1.
                    textbutton "close" action Return()
            hbox:
                style_group "action_editor_a"
                textbutton "clear keyframes" action [SensitiveIf(_viewers.sorted_keyframes), Function(_viewers.clear_keyframes), renpy.restart_interaction]
                textbutton "remove keyframes" action [SensitiveIf(_viewers.time in _viewers.sorted_keyframes), Function(_viewers.remove_keyframes, _viewers.time), renpy.restart_interaction]
                textbutton "move keyframes" action [SensitiveIf(_viewers.time in _viewers.sorted_keyframes), SetField(_viewers, "moved_time", _viewers.time), Show("_move_keyframes")]
                textbutton "last moves" action [SensitiveIf(_last_camera_arguments), Function(_viewers.last_moves), renpy.restart_interaction]
            null height 10
            hbox:
                style_group "action_editor_a"
                xfill False
                textbutton "Images" action [SelectedIf(tab == "images"), Show("_action_editor", tab="images")]
                textbutton "2D Camera" action [SensitiveIf(_3d_layers.keys() == ["master"]), SelectedIf(tab == "2D Camera"), Show("_action_editor", tab="2D Camera")]
                textbutton "3D Layers" action [SelectedIf(tab == "3D Layers"), Show("_action_editor", tab="3D Layers")]
                textbutton "3D Camera" action [SelectedIf(tab == "3D Camera"), Show("_action_editor", tab="3D Camera")]
            if tab == "images":
                hbox:
                    style_group "action_editor_a"
                ...
                hbox:
                    style_group "action_editor_a"
                    textbutton "+" action Function(_viewers.transform_viewer.add_image, layer)
                    for n in state:
                ...
                if _3d_layers.keys() == ["master"] and tab == "3D Camera":
                    label "Please register 3D layers"
                else:
                    hbox:
                        style_group "action_editor"
            ...
            hbox:
                ...
                if tab == "images":
                    if name:
                        textbutton "remove" action [SensitiveIf(name in _viewers.transform_viewer.state[layer]), Show("_action_editor", tab=tab, layer=layer), Function(renpy.hide, name, layer), Function(_viewers.transform_viewer.state[layer].pop, name, layer), Function(_viewers.transform_viewer.remove_keyframes, name=name, layer=layer), _viewers.sort_keyframes]
                        textbutton "clipboard" action Function(_viewers.transform_viewer.put_show_clipboard, name, layer)
                    textbutton "reset" action [_viewers.transform_viewer.image_reset, renpy.restart_interaction]
                elif tab == "2D Camera":
                    textbutton "clipboard" action Function(_viewers.camera_viewer.put_clipboard, True)
                    textbutton "reset" action [_viewers.camera_viewer.camera_reset, renpy.restart_interaction]
                elif tab == "3D Layers":
                    textbutton "clipboard" action Function(_viewers.camera_viewer.put_clipboard, False)
                    textbutton "reset" action [_viewers.camera_viewer.layer_reset, renpy.restart_interaction]
                elif tab == "3D Camera":
                    textbutton "clipboard" action Function(_viewers.camera_viewer.put_clipboard, True)
                    textbutton "reset" action [_viewers.camera_viewer.camera_reset, renpy.restart_interaction]
...
init -1600:
    style action_editor_button is button:
        size_group "action_editor"
        idle_background None
        insensitive_background None
        selected_idle_background Solid("#1d484d")
        selected_hover_background Solid("#3a9099")
    style action_editor_button_text is button_text
    style action_editor_a_button is action_editor_button:
        size_group None
    style action_editor_a_button_text is action_editor_button_text

    style action_editor_label is label xminimum 110
    style action_editor_vbox is vbox xfill True
...
init -1600:
    style input_screen_frame is frame:
        xfill True
        ypos .1
        xmargin .05
        ymargin .05
        background None
    style input_screen_vbox is vbox spacing 30 xfill True
    style input_screen_label is label xalign .5
    style input_screen_hbox is hbox xalign .5
...
screen _warper_selecter(current_warper=""):
    ...
    frame:
        ...
        label "Select a warper function"
        viewport:
            mousewheel True
            edgescroll (100, 100)
        ...
        hbox:
            textbutton "add" action OpenURL("http://renpy.org/wiki/renpy/doc/cookbook/Additional_basic_move_profiles")
            textbutton "close" action Return("")
...
screen _move_keyframes:
    ...
    frame:
        background "#0006"
        has vbox
        textbutton "time: [_viewers.moved_time:>.2f] s" action Function(_viewers.edit_move_all_keyframes)
        ...
        textbutton "close" action Hide("_move_keyframes") xalign .98
...
screen _edit_keyframe(k, int=False):
    ...
    frame:
        ...
        xfill True
        has vbox
        label "KeyFrames" xalign .5
        for v, t, w in check_points:
            if t != 0:
                hbox:
                    textbutton "x" action [Function(_viewers.remove_keyframe, remove_time=t, k=k), renpy.restart_interaction] background None
                    ...
                    textbutton "[t:>.2f] s" action Function(_viewers.edit_move_keyframes, check_points=check_points, old=t)
        ...
        hbox:
            textbutton "loop" action ToggleDict(_viewers.loops, k)
            if k[:8] == "_camera_":
                textbutton "expression" action Function(_viewers.edit_expression, k)
                textbutton "spline" action ToggleDict(_viewers.splines, "camera")
            elif not isinstance(k, tuple):
                textbutton "expression" action Function(_viewers.edit_expression, k)
                textbutton "spline" action ToggleDict(_viewers.splines, k)
            textbutton "close" action Hide("_edit_keyframe") xalign .98
...
init -1600 python in _viewers:
    ...
    show %s onlayer %s""" % (name, layer)
            ...
            try:
                ...
                renpy.notify("Can't open clipboard")
            else:
                renpy.notify('Placed \n"%s"\n on clipboard' % string)
        ...
        def edit_value(self, function, int=False, default=""):
            ...
            if v:
                try:
                    ...
                    renpy.notify("Please type value")
    ...
    def edit_expression(k):
        ...
        try:
            ...
            renpy.notify("This isn't a valid expression")
        renpy.restart_interaction()
